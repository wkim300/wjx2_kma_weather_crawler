...
for swjcity in swjcitynum_list:
    ...
    for swjyear in range(2015, 2020):
        ...
        for swjpage in range(1, 11):
            ...
            swjdata = []
            while len(swjdata) < 1:
                print('Year : ' + str(swjyear) + ' // City : ' +
                      str(swjcity) + ' // Page : ' + str(swjpage) + "of 10")
    ...
    locals()[varname_all2] = [list(locals()[varname_all2])
                              for locals()[varname_all2] in zip(*locals()[varname_all])]
    ...
    for swji in range(1, len(locals()[varname_all2])):
        ...
        current_hournum = datetime.datetime.strptime(
            locals()[varname_all2][swji][0], '%Y-%m-%d %H:%M')
        pre_hournum = datetime.datetime.strptime(
            locals()[varname_all2][swji-1][0], '%Y-%m-%d %H:%M')
    ...
    for swja in range(0, len(locals()[varname_all2 + '_err_index'])):
        ...
        index_miss = locals()[varname_all2 +
                              '_err_index'][swja] + num_prev_inserted
        time_prev = datetime.datetime.strptime(
            locals()[varname_all3][index_miss-1][0], '%Y-%m-%d %H:%M')

        list_insert = []
        for swjb in range(1, num_miss):
            time_insert = time_prev + datetime.timedelta(hours=swjb)
            time_insert_char = datetime.datetime.strftime(
                time_insert, '%Y-%m-%d %H:%M')
